...
class Settings extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['lang'], 'exist', 'skipOnError' => true, 'targetClass' => Language::class, 'targetAttribute' => ['lang' => 'id']],
        ];
    }
    ...
    {
        return $this->hasOne(Language::class, ['id' => 'lang'])->inverseOf('settings');
    }
    ...
    public function generateForm(&$form, $name = null, $value = null){
       ...
       if($this->input == 1){
        ...
        if($this->input == 6){
            ...
            $r .= \common\modules\file\widgets\FileManagerModalSettings::widget([
                'form' => $form,
                'attribute' => $formname,
                'value' => str_replace("NaN,", "", $value),
//                'selected_items_sortable' => str_replace("NaN,", "", $value),
                'id' => 'filesform'.$this->setting_id,
                'relation_name' => 'files',
                'via_relation_name' => 'settingsFiles',
                'multiple'      => true,
                'mime_type'     => 'image',
                'upload_url' => '/file/file/upload'
            ]);
    ...
    {
        return $this->hasMany(Settingsvalues::class, ['setting_id' => 'setting_id']);
    }
    ...
    {
        return $this->hasMany(Values::class, ['value_id' => 'value_id'])->viaTable('settingsvalues', ['setting_id' => 'setting_id']);
    }
        ...
        if($query->count() > 0){
            ...
            if(count($values)){
                ...
                if($setting->input == 6){
                    ...
                    $data = str_replace("NaN,", "", $data);

                    $files_data = explode(",",$data);

                    if (!is_array($files_data)) $files_data = $data;

                    $files = File::find()->where(['id' => $files_data])->all();
                    return $files;
                }
                //input
                if($setting->input == 1){
